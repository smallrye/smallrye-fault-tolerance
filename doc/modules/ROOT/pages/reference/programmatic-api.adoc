...
{smallrye-fault-tolerance} naturally also supports guarding asynchronous actions, using the `java.util.concurrent.CompletionStage` type.
Unlike the declarative API, the programmatic API doesn't support asynchronous actions that return the `java.util.concurrent.Future` type.
...
To enable support of additional asynchronous types, it is enough to include the support libraries, as described in xref:reference/asynchronous.adoc#async-types[Additional Asynchronous Types].

==== Mutiny Support

For example, when the Mutiny support library `io.smallrye:smallrye-fault-tolerance-mutiny` is present, both `Guard` and `TypedGuard` will recognize `Uni` as an asynchronous type and guard it properly.
...
That interface was removed in {smallrye-fault-tolerance} 7.0.
