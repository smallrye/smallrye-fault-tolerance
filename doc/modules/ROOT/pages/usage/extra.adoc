...
One of these annotations may be present on any element (method or class) that also has the `@Retry` annotation.
For example:
...
It is an error to add a backoff annotation to an element that doesn't have `@Retry` (e.g. add `@Retry` on a class and `@ExponentialBackoff` on a method).
It is also an error to add more than one of these annotations to the same element.
