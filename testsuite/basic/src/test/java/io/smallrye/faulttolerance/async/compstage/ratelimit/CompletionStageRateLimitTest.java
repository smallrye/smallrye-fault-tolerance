...
public class CompletionStageRateLimitTest {
    ...
    public void fixedWindowNoSpacing(MyService service) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < TASKS_COUNT; i++) {
            tasks.add(() -> service.fixedWindowNoSpacing().toCompletableFuture().get());
        }
    ...
    public void fixedWindowWithSpacing(MyService service) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < TASKS_COUNT; i++) {
            tasks.add(() -> {
                ...
                return service.fixedWindowWithSpacing().toCompletableFuture().get();
            });
        }
        List<Future<String>> futures = executorService.invokeAll(tasks);
    ...
    public void rollingWindowNoSpacing(MyService service) throws ExecutionException, InterruptedException {
        List<Callable<String>> tasks = new ArrayList<>(TASKS_COUNT);
        for (int i = 0; i < TASKS_COUNT; i++) {
            tasks.add(() -> service.rollingWindowNoSpacing().toCompletableFuture().get());
        }
        List<Future<String>> futures = executorService.invokeAll(tasks);
    ...
    public void rollingWindowWithSpacing(MyService service) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < TASKS_COUNT; i++) {
            tasks.add(() -> {
                ...
                return service.rollingWindowWithSpacing().toCompletableFuture().get();
            });
        }
        List<Future<String>> futures = executorService.invokeAll(tasks);
    ...
    public void smoothWindowNoSpacing(MyService service) throws ExecutionException, InterruptedException {
        List<Callable<String>> tasks = new ArrayList<>(TASKS_COUNT);
        for (int i = 0; i < TASKS_COUNT; i++) {
            tasks.add(() -> service.smoothWindowNoSpacing().toCompletableFuture().get());
        }
        List<Future<String>> futures = executorService.invokeAll(tasks);
    ...
    public void smoothWindowWithSpacing(MyService service) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < TASKS_COUNT; i++) {
            tasks.add(() -> {
                ...
                return service.smoothWindowWithSpacing().toCompletableFuture().get();
            });
        }
        List<Future<String>> futures = executorService.invokeAll(tasks);
