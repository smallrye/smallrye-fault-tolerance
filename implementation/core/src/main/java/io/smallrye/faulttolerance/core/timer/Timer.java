...
public final class Timer {
    ...
    private static final Comparator<TimerTask> TIMER_TASK_COMPARATOR = (o1, o2) -> {
        if (o1 == o2) {
            // two different TimerTask instances are never equal
            return 0;
        }
    ...
    public Timer(Executor executor) {
        ...
        this.thread = new Thread(() -> {
            // TODO can anything here throw and abort the entire timer loop?
            while (running.get()) {
                if (tasks.isEmpty()) {
                    LockSupport.park();
                } else {
    ...
    /**
     * Must be called <i>before</i> the underlying {@code executor} is shut down.
